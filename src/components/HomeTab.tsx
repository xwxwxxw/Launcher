    ...
    <div className="flex-1 w-full h-full overflow-hidden flex flex-row relative">
      ...
      <div className="flex-1 px-10 py-12 overflow-y-auto scrollbar-none h-full relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-14">
          <StatCard icon={<Package className="text-purple-400" size={20} strokeWidth={2} />} title="Установлено модов" value={modsCount} trend="В выбранной сборке" onClick={() => onNavigate('mods')} />
          ...
          <StatCard icon={<Cpu className="text-purple-400" size={20} strokeWidth={2} />} title="Выделено памяти" value={(ram >= 1024) ? (ram / 1024).toFixed(ram % 1024 === 0 ? 0 : 1) + " GB" : ram + " MB"} trend={ram < 3072 ? "Рекомендуется больше" : ram > 8192 ? "Слишком много" : "Оптимально"} onClick={() => onNavigate('settings', 'ram')} />
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          <QuickAction 
            ...
            icon={<FolderTree size={24} className="mb-4 text-purple-400" strokeWidth={1.5} />}
          />
          <QuickAction 
